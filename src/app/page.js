...
const SCHEDULE_TIMES = [
  ...
  // Popodnevna (koristimo iste slotove samo pomerene za +6h u logici ili fiksno)
  ...
  { num: 0, label: "ОДМОР", start: "18:25", end: "18:30" },
  { num: 6, label: "18:30", end: "19:15" },
  { num: 7, label: "19:15", end: "20:00" },
];
...
export default function SchoolTV() {
  ...
  const status = useMemo(() => {
    ...
    // Odredi koja je smjena trenutno u školi
    const activeShiftInSchool = isMorning ? morningShift : (morningShift === 'Parna' ? 'Neparna' : 'Parna');
    
    // Filtriraj raspored samo za trenutni čas i trenutnu smjenu
  ...
  if (emergency === "УЗБУНА") {
    return (
      <div className="h-screen w-screen bg-red-700 flex flex-col items-center justify-center text-white">
        ...
        <h1 className="text-[20vh] font-black">УЗБУНА</h1>
      </div>
    );
  }
    ...
    <div className="h-screen w-screen bg-[#F1F5F9] p-[1.5vh] flex flex-col font-sans overflow-hidden">
      ...
      <div className="h-[12vh] bg-white rounded-[2.5rem] shadow-sm flex justify-between items-center px-[5vh] mb-[1.5vh] border border-white">
        <div className="flex items-center gap-[4vh]">
          <img src="/logo.png" alt="Logo" className="h-[8vh]" />
          <div>
      ...
      <div className="flex flex-1 gap-[1.5vh] min-h-0">
        ...
        <div className="w-[60%] bg-white rounded-[4rem] shadow-xl flex flex-col items-center justify-center border border-white relative overflow-hidden">
             <div className="relative h-[60vh] w-[60vh] flex items-center justify-center">
                ...
                <div className="text-center z-10">
                    ...
                    <h2 className="text-[11vh] font-black leading-none text-slate-800 italic">{status.activeSlot.label}</h2>
                    {status.activeSlot.num > 0 && <div className="text-[5vh] font-black text-blue-600 mt-4">{Math.round(status.progress)}%</div>}
                </div>
             </div>
        </div>
        ...
        <div className="w-[40%] bg-white rounded-[4rem] shadow-xl flex flex-col relative overflow-hidden border border-white">
          <div className="flex-1 p-[5vh]">
            
            {/* SLAJD 0: OBAVEŠTENJA + VREME */}
            {activeTab === 0 && (
              <div className="animate-in fade-in duration-500 h-full flex flex-col">
                <div className="flex justify-between items-center mb-10 bg-slate-50 p-6 rounded-[2rem]">
                   <div>
                      <h3 className="text-[4vh] font-black text-slate-800 leading-none">{weather.temp}°C</h3>
                      <p className="text-slate-400 font-bold uppercase tracking-widest text-sm">Тренутно време</p>
                   </div>
            ...
            {/* SLAJD 1: RASPORED KABINETA ZA OVAJ ČAS */}
            {activeTab === 1 && (
              <div className="animate-in slide-in-from-right duration-500 h-full">
                <h3 className="text-[3vh] font-black text-blue-600 mb-6 uppercase flex items-center gap-3"><BookOpen /> Распоред кабинета</h3>
                <p className="text-slate-400 mb-6 font-bold uppercase italic">За тренутни: {status.activeSlot.label}</p>
                <div className="grid grid-cols-1 gap-3">
                  {status.currentRooms.length > 0 ? status.currentRooms.map((r, i) => (
                    <div key={i} className="flex justify-between items-center bg-blue-50 p-5 rounded-[2rem] border border-blue-100">
                      <span className="text-[2.8vh] font-black text-slate-700">{r.class_name}</span>
                      <span className="bg-white px-6 py-1 rounded-full font-black text-blue-600 shadow-sm">{r.room}</span>
                    </div>
                  )) : <div className="text-center py-20 text-slate-300 font-black uppercase">Нема података за овај час</div>}
                </div>
              </div>
            )}
            ...
            {activeTab === 2 && (
              <div className="animate-in slide-in-from-right duration-500 h-full">
                <h3 className="text-[3vh] font-black text-pink-500 mb-10 uppercase flex items-center gap-3"><Cake /> Срећан рођендан</h3>
                <div className="space-y-4">
                  {birthdays.map((b, i) => (
            ...
            {activeTab === 3 && (
              <div className="animate-in zoom-in duration-500 h-full flex flex-col items-center justify-center text-center">
                ...
                <p className="text-[3.5vh] font-black italic text-slate-800 leading-tight">"{quotes[0]?.text}"</p>
              </div>
            )}
